...
const OUTPUT_FILE = path.resolve(__dirname, '../md/components/magicui-full.md');
const TIMEOUT = 30000;
const scraped = [];
const skipped = [];
...
async function getLinks(page) {
  ...
  const links = await page.$$eval('a[href*="/docs/components/"]', els =>
    els.map(e => ({
      href: e.href.startsWith('http') ? e.href : 'https://magicui.design' + e.getAttribute('href'),
      text: e.innerText.trim()
    })).filter(e => e.text && e.href.includes('/docs/components/') && !e.href.endsWith('/docs/components'))
  );
...
async function main() {
    ...
    try {
      ...
      if (code) {
        const deps = (code.match(/^import .+ from ['"]([^'"]+)['"]/gm) || [])
          ...
          .filter(Boolean);
        scraped.push({ name: `MagicUI: ${link.text}`, url: link.href, code, deps: [...new Set(deps)] });
  ...
  let md = `# MagicUI — Full Component Source Code\n\n> Scraped: ${new Date().toISOString()}\n> Total: ${scraped.length}\n\n---\n\n`;
