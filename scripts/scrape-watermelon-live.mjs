...
async function scrapeWatermelon(browser) {
  console.log('\n=== Watermelon UI ===');
  const baseUrl = 'https://ui.watermelon.sh';

  // Try /components then root
      ...
      return els
        ...
        .filter((h) => {
          try {
            const u = new URL(h);
            return (
              u.hostname.includes('watermelon.sh') &&
              u.pathname.length > 1 &&
              !seen.has(h) &&
              seen.add(h)
            );
  ...
  if (!links || links.length === 0) {
    console.error('Watermelon UI: no links found');
    return '';
  }

  // Filter to component-like paths
...
(async () => {
  ...
  try {
    const content = await scrapeWatermelon(browser);

    write(
      `${OUT_DIR}/watermelon-full.md`,
      `# Watermelon UI — Full Component Source\n\nScraped: ${new Date().toISOString()}\n\n---\n\n${content || '_No content extracted_'}`
    );
  } finally {
    await browser.close();
  }
  ...
  try {
    execSync('git add md/components/watermelon-full.md scripts/scrape-watermelon-live.mjs', {
      cwd,
      stdio: 'inherit',
    });
    execSync('git commit -m "Add scraped source: watermelon UI components"', {
      cwd,
      stdio: 'inherit',
    });
